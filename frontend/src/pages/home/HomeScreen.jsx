  ...
  return (
    <>
      <div className="relative h-screen text-white ">
        ...
        {/* loading page while image loading */}

        {imageLoading && (<div className="absolute top-0 left-0 w-full h-full bg-black/70 flex items-center justify-center -z-10 shimmer" />
        )}
        <img src={ORIGINAL_IMG_BASE_URL + trendingContent?.backdrop_path} onLoad={() => setImageLoading(false)} alt="img" aria-hidden="true" className="absolute top-0 left-0 w-full h-full object-cover -z-50" />
        <div className=" absolute top-0 left-0 w-full h-full bg-black/50 -z-50" />
        <div className="absoloute top-0 left-0 w-full h-full flex flex-col justify-center px-8 md:px-16 lg:px-32">
          ...
          <div className="max-w-2xl">
            ...
            <p>{trendingContent?.release_date?.split("-")[0] || trendingContent?.first_air_date?.split("-")[0]} |
          ...
          <div className="flex mt-8">
            <Link to={`watch/${trendingContent?.id}`} className="bg-white hover:bg-white/80 text-black font-bold py-2 px-2 rounded mr-4 flex items-center">
